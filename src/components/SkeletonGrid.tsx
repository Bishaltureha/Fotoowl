...
const SkeletonGrid: FunctionComponent<SkeletonGridProps> = ({ count = 18 }) => {
  ...
  const renderSkeletonItem = (index: number) => (
    <View key={index} testID={`skeleton-item-${index}`} style={styles.subcontainer}>
      <View style={styles.skeletonContent}>
        <Animated.View
          testID={`shimmer-overlay-${index}`}
  ...
  return (
    <View testID={`skeleton-grid-container`} style={styles.container}>
